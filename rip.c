...
void request_Handle(struct in_addr stSourceIp)
{
    ...
    while(now != NULL)
    {
        ...
        {
            if (!directConnect(now->stIpPrefix, stSourceIp, now->uiPrefixLen))
            {
        ...
        while(now != NULL)
        {
            if(now->stIpPrefix.s_addr == ripReceivePkt->RipEntries[i].stAddr.s_addr)
            {
                ...
                }else if(my >= his)
                {
                    ...
                    {
                        needUpdate = 1;
                        route_SendForward(DelRoute, now);
                    }
                    now->uiMetric = ntohl(his);
...
void route_SendForward(unsigned int uiCmd, TRtEntry *pstRtEntry)
{
    ...
    /*
    for (int i = 0; i < interCount; i++)
        if (pstRtEntry->stIpPrefix.s_addr == (pcLocalAddr[i].s_addr & pcLocalMask->s_addr))
        {
            printf("This is a direct connected route!");
            selfrt->selfnexthop = pcLocalAddr[i];
        }
*/
    selfrt->cmdnum        = uiCmd;

    printf("        ipPrefix=%16s ", inet_ntoa(selfrt->selfprefix));
    printf("nextHop=%16s", inet_ntoa(selfrt->selfnexthop));
...
void rippacket_Update(struct in_addr pcLocalAddr)
{
    ...
    while(now != NULL)
    {
        ...
        {
            if(!directConnect(now->stIpPrefix, pcLocalAddr, now->uiPrefixLen) && !directConnect(pcLocalAddr, now->stNexthop, now->uiPrefixLen))
            {
                ...
                ripSendUpdPkt->RipEntries[ripSendUpdPkt->ripEntryCount].uiMetric = (now->isValid == ROUTE_NOTVALID)?htonl(RIP_INFINITY):now->uiMetric;//poison reverse
